    ...
    for r, d, f in os.walk(models_directory):
        for file in f:
            if ".pt" in file:
                ...
                # torch.hub.load('path/to/yolov5', 'custom', path='path/to/best.pt', source='local')  # local repo
                dictOfModels[os.path.splitext(file)[0]] = torch.hub.load(
                    'ultralytics/yolov5', 'custom', path="./models_train/best_weights.pt", force_reload=True)
